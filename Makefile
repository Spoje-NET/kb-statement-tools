...
buildimage:
	docker build -f Containerfile -t spojenet/kb-statement-downloader:latest .

buildx:
	docker buildx build -f Containerfile . --push --platform linux/arm/v7,linux/arm64/v8,linux/amd64 --tag spojenet/kb-statement-downloader:latest

drun:
	docker run --env-file .env spojenet/kb-statement-downloader:latest
